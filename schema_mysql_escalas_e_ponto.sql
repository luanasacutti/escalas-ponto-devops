CREATE DATABASE IF NOT EXISTS escala_ponto_devops_php
CHARACTER SET utf8mb4
COLLATE utf8mb4_unicode_ci;

USE escala_ponto_devops_php;
...
CREATE TABLE escalas (
    ...
    CONSTRAINT fk_escala_turno
        ...
        ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT uq_funcionario_data_escala UNIQUE (funcionario_id, data_escala)
);
...
CREATE TABLE registros_ponto (
    ...
    fim_intervalo DATETIME,
    saida DATETIME,
    horas_trabalhadas DECIMAL(5,2) DEFAULT 0,
    status ENUM('aberto', 'fechado', 'ajuste_pendente', 'ajustado') NOT NULL DEFAULT 'aberto',
    criado_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_ponto_funcionario
        FOREIGN KEY (funcionario_id) REFERENCES funcionarios(id)
        ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT uq_ponto_funcionario_data UNIQUE (funcionario_id, data_ponto)
);
...
CREATE TABLE ajustes_ponto (
    ...
    funcionario_id INT NOT NULL,
    tipo_ajuste ENUM('entrada', 'inicio_intervalo', 'fim_intervalo', 'saida') NOT NULL,
    justificativa TEXT NOT NULL,
    status ENUM('pendente', 'aprovado', 'recusado') NOT NULL DEFAULT 'pendente',
    aprovado_por INT,
    criado_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_ajuste_ponto
        FOREIGN KEY (registro_ponto_id) REFERENCES registros_ponto(id)
        ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT fk_ajuste_funcionario
        FOREIGN KEY (funcionario_id) REFERENCES funcionarios(id)
        ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT fk_ajuste_aprovador
